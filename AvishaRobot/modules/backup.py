...
import AvishaRobot.modules.sql.blacklist_sql as blacklistsql

import AvishaRobot.modules.sql.locks_sql as locksql
import AvishaRobot.modules.sql.notes_sql as sql

import AvishaRobot.modules.sql.rules_sql as rulessql
from AvishaRobot import EVENT_LOGS,OWNER_ID, SUPPORT_CHAT, dispatcher
from AvishaRobot.__main__ import DATA_IMPORT
from AvishaRobot.modules.connection import connected
from AvishaRobot.modules.helper_funcs.alternate import typing_action
from AvishaRobot.modules.helper_funcs.chat_status import user_admin
from AvishaRobot.modules.sql import disable_sql as disabledsql
...
def import_data(update, context):
    ...
    if msg.reply_to_message and msg.reply_to_message.document:
        ...
        if conn:
            ...
            text = "❖ ʙᴀᴄᴋᴜᴘ ᴅᴀᴛᴀ ғᴜʟʟʏ ʀᴇsᴛᴏʀᴇᴅ ᴏɴ ➥ *{}*.".format(chat_name)
        else:
            text = "❖ ʙᴀᴄᴋᴜᴘ ᴅᴀᴛᴀ ғᴜʟʟʏ ʀᴇsᴛᴏʀᴇᴅ."
...
def export_data(update, context):
    ...
    if conn:
        ...
        if update.effective_message.chat.type == "private":
            update.effective_message.reply_text("❖ ᴛʜɪs ᴄᴏᴍᴍᴀᴅs ᴜsᴇ ᴏɴʟʏ ɪɴ ɢʀᴏᴜᴘ.")
            return ""
        chat = update.effective_chat
        chat_id = update.effective_chat.id
        # chat_name = update.effective_message.chat.title
    ...
    with open("AvishaRobot{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("AvishaRobot{}.backup".format(chat_id), "rb"),
        caption="❖ sᴜᴄᴄᴇssғᴜʟʟʏ ᴇxᴘᴏʀᴛᴇᴅ ʙᴀᴄᴋᴜᴘ ⏤͟͟͞͞★\n\n● ᴄʜᴀᴛ ➥ `{}`\n● ᴄʜᴀᴛ ɪᴅ ➥ `{}`\n● ᴏɴ ➥ `{}`\n\n❖ ɴᴏᴛᴇ ➥ ᴛʜɪs `๛ᴀ ᴠ ɪ s ʜ ᴀ ࿐` ᴡᴀs sᴘᴇᴄɪᴀʟʟʏ ᴍᴀᴅᴇ ғᴏʀ ɴᴏᴛᴇs.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("AvishaRobot{}.backup".format(chat_id))  # Cleaning file
